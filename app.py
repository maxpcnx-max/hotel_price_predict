...
def load_system_engine():
    ...
    # Metrics
    xgb_metrics = {'mae': 1112.79, 'r2': 0.7256}
    lr_metrics = {'mae': 1162.27, 'r2': 0.7608}
...
if not st.session_state['logged_in']:
    ...
    def show_dashboard_page():
        ...
        # 1. Pie Chart: ห้องไหนจองเยอะที่สุด (ตามที่ขอ)
        st.subheader("1. สัดส่วนห้องที่จองเยอะที่สุด (Booking Proportion by Room Type)")
        room_count = df['Target_Room_Type'].value_counts().reset_index()
        room_count.columns = ['Room_Type', 'Count']
        fig_pie = px.pie(room_count, values='Count', names='Room_Type', 
                         title='สัดส่วนจำนวนการจองแยกตามประเภทห้อง', 
        ...
        with col_g1:
            # 2. Bar Chart: รายได้แยกตามห้อง (ของเดิม)
            st.markdown("**💸 สัดส่วนรายได้ตามประเภทห้อง (Revenue)**")
            rev = df.groupby('Target_Room_Type')['Price'].sum().reset_index()
            fig_rev = px.bar(rev, x='Target_Room_Type', y='Price', color='Price', text_auto='.2s')
            st.plotly_chart(fig_rev, use_container_width=True)

        with col_g2:
            # 3. Bar Chart: กราฟแถม (ตามที่ขอ) -> แยกตามช่องทางการจอง
            st.markdown("**🌐 ยอดจองแยกตามช่องทาง (Reservation Channel)**")
            res_count = df['Reservation'].value_counts().reset_index()
            res_count.columns = ['Channel', 'Count']
            fig_res = px.bar(res_count, x='Channel', y='Count', color='Count', text='Count',
                             color_continuous_scale='Magma')
            st.plotly_chart(fig_res, use_container_width=True)

        # 4. Monthly Trend
        st.divider()
        st.markdown("**📈 แนวโน้มรายได้รายเดือน (Monthly Revenue)**")
